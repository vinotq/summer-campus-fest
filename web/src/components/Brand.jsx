...
export function Mark({ size = 24, fill = 'gradient', style }) {
  const id = React.useId()
  const useGrad = fill === 'gradient'
  const fillVal = useGrad ? `url(#${id})` : fill
  return (
    <svg width={size} height={size} viewBox="0 0 315 300"
      ...
      <path fillRule="evenodd" clipRule="evenodd" fill={fillVal}
...
export function MarkField({ count = 28, opacity = 0.08, color = '#814387', style }) {
  ...
  const items = Array.from({ length: count }, (_, i) => ({
    ...
    s: 14 + rng(i * 3) * 36, r: (rng(i * 5) - 0.5) * 60, key: i,
  }))
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', ...style }}>
      {items.map(it => (
        <Mark key={it.key} size={it.s} fill={color} style={{
          position: 'absolute', left: `${it.x}%`, top: `${it.y}%`,
          transform: `translate(-50%,-50%) rotate(${it.r}deg)`, opacity,
        }} />
      ))}
    </div>
  )
